...
def split_audio_for_telegram(
    ...
    *,
    ...
    """Split an audio file into the minimum number of size-constrained parts."""
    if not input_path.exists():
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    resolved_output_suffix = (output_suffix or input_path.suffix or ".mp3").lower()

    if input_path.stat().st_size <= max_size_bytes:
        output_path = output_dir / f"part_0{resolved_output_suffix}"
    ...
    for part_count in range(minimum_parts, maximum_parts + 1):
        outputs = _split_audio_into_parts(
            ...
            ffmpeg_path=ffmpeg_path,
            output_suffix=resolved_output_suffix,
        )
    ...
    raise RuntimeError("Audio split failed: could not fit all parts below the size limit.")
...
def _split_audio_into_parts(
    ...
    *,
    ...
    for index in range(part_count):
        ...
        output_path = output_dir / f"part_{index}{output_suffix}"
        if output_path.exists():
            output_path.unlink()
